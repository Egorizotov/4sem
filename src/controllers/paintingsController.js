...
const createPainting = (req, res) => {
    const { src, title, subtitle, text, notification, running_line } = req.body;

    if (!src || !title || !text) {
        return res.status(400).json({ error: 'Не все обязательные поля заполнены (src, title, text)' });
    }

    const newPainting = paintingsService.createNewPainting({ src, title, subtitle, text, notification, running_line });
